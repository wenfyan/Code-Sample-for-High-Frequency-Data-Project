...
def generate_cal_Breadth(df, delta1, delta2):
    """
    Generate calendar Breadth values for trades and quotes in the given DataFrame.Which is mainly about the number of transactions in an intended
    time interval.

    This function calculates the calendar Breadth values for trades in the provided DataFrame (`df`).
    The calculation is based on the specified time deltas `delta1` and `delta2`.
    ...
    -------
    pandas.Series
        A pandas Series containing the calculated calendar Breadth values.
    """
...
def generate_cal_Immediacy(df, delta1, delta2):
    return (delta2-delta1) / generate_cal_Breadth(df, delta1, delta2)

def generate_cal_AvgVolume(df, delta1, delta2):
    return generate_cal_VolumeAll(df, delta1, delta2) / generate_cal_Breadth(df, delta1, delta2)
...
def generate_trans_Immediacy(df, delta1, delta2):
    return (delta2-delta1) / generate_trans_Breadth(df, delta1, delta2)

def generate_trans_AvgVolume(df, delta1, delta2):
    return generate_trans_VolumeAll(df, delta1, delta2) / generate_trans_Breadth(df, delta1, delta2)
...
def parent_generator_ret_imb(df, deltas, mode='calendar'):
    ...
    cal_feature_mapping = {
        'Bredth': generate_cal_Breadth,
        'Immediacy': generate_cal_Immediacy,
        'VolumeAll': generate_cal_VolumeAll,
        'AvgVolume': generate_cal_AvgVolume,
        'VolumeMax': generate_cal_VolumeMax,
    ...
    trans_feature_mapping = {
        'Bredth': generate_trans_Breadth,
        'Immediacy': generate_trans_Immediacy,
        'VolumeAll': generate_trans_VolumeAll,
        'AvgVolume': generate_trans_AvgVolume,
        'VolumeMax': generate_trans_VolumeMax,
    ...
    vol_feature_mapping = {
        'Bredth': generate_vol_Breadth,
        'Immediacy': generate_vol_Immediacy,
        'VolumeAll': generate_vol_VolumeAll,
        'AvgVolume': generate_vol_AvgVolume,
        'VolumeMax': generate_vol_VolumeMax,
        'Lambda': generate_vol_lambda,
